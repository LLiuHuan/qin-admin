...
import type { CustomAlovaConfig, RequestMethod } from './types';
...
class AlovaClient {
   ...
   * 请求拦截器数组
   */
  public requestInterceptor: RequestMethod<AlovaGenerics>[] = [];
  ...
  constructor(
    options: CustomAlovaConfig<AlovaGenerics>,
    ...
    const responded = {
      ...
      onSuccess: async (response: Response, method: Method<AlovaGenerics>) => {
        let result: any = null;
      ...
      onError: async (err: Error, method: Method<AlovaGenerics>) => {
      ...
      // 接收当前请求的method实例
      onComplete: async (method: Method<AlovaGenerics>) => {
        // 处理请求完成逻辑
    ...
    this.instance = createAlova({
      ...
      requestAdapter: (options.requestAdapter as any) ?? adapterFetch(),
      statesHook: VueHook,
   ...
   * @param method 拦截器方法
   */
  public addRequestInterceptor(method: RequestMethod<AlovaGenerics>) {
  ...
  public addResponseSuccessInterceptor(
    method: RespondedHandler<AlovaGenerics>,
  ) {
...
export * from 'alova';
// 解决冲突
export type { Progress as AlovaProgress } from 'alova';
export type { Progress } from 'alova';
export type { Progress as AlovaClientProgress } from 'alova/client';
