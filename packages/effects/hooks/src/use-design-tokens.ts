...
export function useElementPlusDesignTokens() {
  ...
  watch(
    ...
    () => {
      ...
      updateCSSVariables(variables, `__arco_design_styles__`);
    },
    { immediate: true },
  );
}
  ...
  watch(
    ...
    () => {
      ...
      const variables: Record<string, string> = {
        ...
        '--border-radius-small': getCssVariableValue('--radius', false),
        '--border-radius-medium': getCssVariableValue('--radius', false),
        '--border-radius-large': getCssVariableValue('--radius', false),
