...
import { useI18n } from '@qin/locales';
...
import { TableSizeEnum } from './types';

defineOptions({ name: 'QinTableHeader' });

const props = withDefaults(defineProps<Props>(), {
  showZebra: true,
  showBorder: true,
  showHeaderBackground: true,
  fullClass: 'qin-page-view',
...
<template>
  <div class="flex-cb max-md:!block" id="qin-table-header">
    <div class="flex-wrap">
      <slot name="left"></slot>
    </div>

    <div class="flex-c max-md:mt-3 max-sm:!hidden md:justify-end">
      <div
      ...
      >
        <IconifyIcon
          ...
          :class="loading && isManualRefresh ? 'text-g-600 animate-spin' : ''"
        />
      </div>

      <ElDropdown v-if="shouldShow('size')" @command="handleTableSizeChange">
        <div class="button">
          <IconifyIcon icon="ri:arrow-up-down-fill" />
        </div>
        <template #dropdown>
          <ElDropdownMenu>
            <div
      ...
      >
        <template #reference>
          <div class="button">
            <IconifyIcon icon="ri:align-right" />
          </div>
        </template>
        <div>
            ...
            >
              ...
              >
                ...
                <ElCheckbox
                  ...
                  @update:model-value="
                    (val: any) => updateColumnVisibility(item, val)
                  "
                  :disabled="item.disabled"
                  class="min-w-0 flex-1 [&_.el-checkbox__label]:overflow-hidden [&_.el-checkbox__label]:text-ellipsis [&_.el-checkbox__label]:whitespace-nowrap"
                >
                  {{
                    item.label ||
